...
const api = {
      bgw: {
        ...
        api: 'https://jsfapi.netlify.app/.netlify/functions/bgw?search=rom1:17',
        },
      bst: {
...
// update api url
   ...
   const my_api = new URL(this.api);
   this.endpoint = my_api.origin + my_api.pathname +'?search='+ this.canon;
  }

async api_(){
  try{
    this._body = await 
      axios.get(this.endpoint).then(x=>x.data);
